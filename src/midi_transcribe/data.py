...
def load_state4_piece(path: Path) -> Tuple[torch.Tensor, torch.Tensor]:
    obj = torch.load(path, map_location="cpu")
    features = obj["features"].to(dtype=torch.float32).transpose(0, 1).contiguous()  # [T, 128]
    targets = obj["targets_state4"].to(dtype=torch.long).contiguous()  # [T, 88]
    return features, targets
...
def iterate_state4_batches(
    ...
    for path in file_order:
        features, targets = load_state4_piece(path)
        total_frames = features.shape[0]
        starts = segment_starts(total_frames, segment_frames, segment_stride)
